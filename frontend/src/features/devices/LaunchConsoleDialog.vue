...
// 远程连接启动流程（M3T4/M4T4/M5T4/M5T5 / PLT-09，API_CONTRACT.md §7）：
// console.kvm.open（KVM 控制台）、console.dsm.open（DSM 管理界面）与
// console.web.open（交换机 Web 管理界面）共用同一一次性票据流程：点击"打
// 开"→ 同步占位新标签页（避免弹窗拦截）→ POST /devices/{id}/launches →
// 成功后把新标签页导航到返回的一次性消费 URL（GET /launches/{id} 返回
// HTML 自动跳转页，厂商 URL 不进入 SPA 状态，且每次只可读取一次；
// console.web.open 描述符来自设备声明过的 Web 管理入口，无凭据注入 —
// ADR-006）。console.ssh.open / console.telnet.open（M5T4）签发浏览器
// 终端票据：SPA 内跳转终端页 /terminal/sessions/{ticket} 并建立 WebSocket
// （服务端校验 Origin + 会话 Cookie + 一次性票据）。Telnet 为弱协议：开启
// 前持续提示明文风险（SECURITY.md §6 / ADR-007）。错误按稳定错误码渲染
// 中文提示；其他错误走 ErrorDetail。
...
/** 目标类型（能力键派生）：DSM/Web 管理界面 / KVM 图形控制台 / SSH/Telnet 终端。 */
const isDsmConsole = computed(() => props.capabilityKey === 'console.dsm.open');
const isWebConsole = computed(() => props.capabilityKey === 'console.web.open');
...
function confirmHint(): string {
  ...
  if (isWebConsole.value) {
    return '平台将签发 60 秒有效的一次性启动票据，并在新标签页打开设备声明的 Web 管理界面'
      + '（平台不注入密码，设备页面可能要求再次登录）。';
  }
...
function notConfiguredHint(): string {
  ...
  if (isWebConsole.value) {
    return '设备当前未声明可用的 Web 管理入口（需在设备连接配置中声明 Web 管理协议与端口）。'
      + '可重新执行连接测试刷新能力后再试。';
  }
  if (isDsmConsole.value) {
...
function dialogTitle(): string {
  ...
  if (isWebConsole.value) {
    return `打开设备 Web 管理界面：${props.capabilityKey}`;
  }
