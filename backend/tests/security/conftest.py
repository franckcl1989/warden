"""Shared fixtures for the M1 gate security suite (tests/security).

Mirrors the pattern from tests/api/conftest.py (M1T6 brief §1): the app is
built from an explicit WardenSettings whose allowed management CIDR is the
TEST-NET-1 range 192.0.2.0/24 — loopback, link-local (incl. 169.254.169.254),
RFC 1918 and the container ranges all stay OUTSIDE it, so the SSRF tests are
meaningful against the real policy.
"""
