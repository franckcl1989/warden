"""Inspur iBMC overlay — adapter_key ``server.inspur_ibmc`` (M3T5).
...
device self-identifies as an Inspur server (hardware-targets.json target
``server.inspur_ibmc``, certification_scope=management_family,
exact_model_must_be_recorded). Inspur iBMC and xFusion iBMC are separate
...
sharing only where the documented behavior is identical AND flagged).
...
- **simulator-verified** (covered by the Inspur vendor-profile fixture
  tests, tests/fixtures/redfish/system-inspur-healthy.json + memory/drive
  ...
  - power.cycle mapping: inherited from the common adapter rule
    ...
    the common two-type rule with advertised-type refusal stays; a
    certified iBMC pin lands with real-device evidence;
...
- **experimental / unsupported-until-real-hardware** (no 真机 or
  ...
  - real iBMC OEM member names for ECC/SMART/predictive-failure/RAID and
    the Inspur OEM namespace (the vendor profile only emits the generic OEM
    members under the Inspur namespace; nothing vendor-only is parsed until
    a sanitized 真机 fixture exists);
  ...
  - the Inspur public Redfish/iBMC documentation pages were NOT reachable
    from this environment (support pages require login/JS), so no mapping
    below is doc-verified.
- **doc URLs consulted** (2026-09-03):
  - https://www.inspur.com/ (product/support portal; no public iBMC Redfish
    guide reachable);
...
IDENTITY_MANUFACTURER_KEYWORD = "inspur"
IDENTITY_MANUFACTURER_EXAMPLE = "Inspur"

#: Vendor identity requirements (probe gate; shown in stage detail on
#: mismatch). Simulator-verified against the Inspur vendor profile — the
...
vendor_identity_checks: tuple[str, ...] = (
    "ComputerSystem.Manufacturer 必须包含 'inspur'（iBMC 自述如 'Inspur'）",
)
...
class InspurIbmcAdapter(RedfishCommonAdapter):
    """Inspur iBMC Redfish adapter (target server.inspur_ibmc, M3T5).
    ...
    def _probe_identity_checks(self, system: RedfishResource) -> tuple[str, ...]:
        # basis: tests/fixtures/redfish/system-inspur-healthy.json (simulator-verified)
