...
| 6 | 自动化测试与真机证据满足测试策略 | done（自动化）/ blocked-with-evidence（真机） | 后端 2111 通过 / 78 跳过、前端 163 通过（M6T2b 连续两轮全绿）；M6T3b 全量实测：后端 2136 通过 / 78 跳过 / 0 失败、前端 25 文件 / 177 通过（含系统状态/维护模式/摄取心跳测试）；M6T4b 全量实测：后端 2143 通过 / 78 跳过 / 0 失败；**M6T5 最终门禁全量实测（HEAD `3e239f9`，2026-09-07）：后端 2144 通过 / 78 跳过 / 0 失败 / 0 错误（39:16）、前端 25 文件 / 177 通过**；认证矩阵 306/306 `not_started` |
...
逐项核对 51 条原始需求并运行真机矩阵严格门禁覆盖 306 个目标/能力组合。实际执行（2026-09-07）：M6T5 最终门禁复核重跑两个命令，输出与本记录一致（非严格退出 0；`-RequireReleaseReady` 退出 1，306 条记录全部 not_started、同一组缺失证据错误）。
...
1. 部署环境依赖的发布项（compose 运行时、镜像 SBOM/签名/摘要、离线包、安装/升级演练 + python/npm 依赖漏洞扫描，见 KNOWN_LIMITATIONS §3）——M6T5 复核后仍 BLOCKED-here；执行后复核第 1 节第 5 项。
2. 现场真机认证：发布负责人对 10 目标/306 记录逐能力执行认证并回填矩阵后复核第 1 节第 1/2/3 项；这是 `0.1.0-rc.1` 升级为 `0.1.0` 前的软件外门禁。

历史关闭项：M6T3 发现的 PLT-08 `GET /system/status` 缺口（无实现、无契约响应结构、无台账延期记录）已由 M6T3b 补齐交付（commit `cf375a9`）——本清单第 1 节第 6 项与第 2 节第 1/7/8 项随之转为 done。M6T5 最终门禁（2026-09-07，复核记录见 `M6T5-report.md`）：全量套件后端 2144 通过 / 78 跳过 / 0 失败、前端 25 文件 / 177 通过；check-design 通过、check-traceability 51/51 全绿（closeout.json `ok: true`）、OpenAPI 规则 50/50、迁移全周期 upgrade→downgrade→upgrade（head `0016_metric_dedupe_partial`）通过、矩阵非严格通过 / 严格门禁按预期退出 1；DoD（PROJECT_SPEC §7 + IMPLEMENTATION_PLAN §8）逐项复核维持 done（软件侧）+ blocked-with-evidence（真机/部署项）。
